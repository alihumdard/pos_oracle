...
<div class="py-10">
    ...
    <div class="bg-white shadow-lg rounded-lg mb-6 overflow-hidden">
        ...
        <form id="productForm"
            action="{{ $editing ? route('transaction.update', $editTransaction->id) : route('transaction.sales') }}"
                ...
                <div>
                    ...
                    <select id="productSelect" name="product_id"
                        ...
                        <option value="{{ $product->id }}"
                            ...
                            Item Code({{ $product->item_code }}) -- Product Name ({{ $product->item_name }}) -- Original
                            Price ({{ $product->original_price }}) -- Sale Price ({{ $product->selling_price }})
                        </option>
                        @endforeach
                    </select>
                </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mt-6">
                <div>
                    ...
                    <input type="number" id="quantity" name="quantity"
                        value="{{ $editing ? $editTransaction->quantity : old('quantity', 1) }}"
                ...
                <div>
                    ...
                    <input type="number" id="discount" name="discount"
                        value="{{ $editing ? $editTransaction->discount : old('discount', 0) }}"
                        class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md py-2 px-3 border"
                        min="0">
                </div>
                ...
                <div>
                    ...
                    <input type="text" id="service" name="service"
                        value="{{ $editing ? $editTransaction->service_charges : old('service', 0) }}"
                        class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md py-2 px-3 border"
                        min="0">
                </div>
            </div>
            ...
            <div class="mt-6 flex space-x-3">
                <button type="button" id="saveTransactionButton"
                    class="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-gradient-to-r from-blue-600 to-blue-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 transition-colors duration-200">
                    <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"
                        xmlns="http://www.w3.org/2000/svg">
                ...
                <button type="button" id="submitTransactionButton"
    ...
    <div class="bg-white shadow-lg rounded-lg overflow-hidden">
        ...
        <div class="p-6">
            <form id="saleForm" action="{{ isset($sale) ? route('sale.update', $sale->id) : route('sale.store') }}"
                method="POST">
                @csrf
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200" id="tableHolder">
                            ...
                            <tr data-id="{{ $transaction->id }}" class="hover:bg-gray-50">
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                    ...
                                    <input type="hidden" value="{{ $transaction->id }}" name="transaction_id[]"
                                        id="transaction_id_{{ $transaction->id }}">
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                                    {{ $transaction->products->item_name ?? ' '}}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                    {{ $transaction->products->selling_price ?? ' ' }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                    {{ $transaction->quantity ?? ' ' }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                    {{ $transaction->discount ?? ' ' }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                    {{ $transaction->service_charges ?? ' ' }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                                    {{ $transaction->total_amount ?? ' ' }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                    <a href="javascript:void(0)"
                                        class="text-yellow-600 hover:text-yellow-900 mr-3 edit-sale"
                                        data-id="{{ $transaction->id }}" data-product="{{ $transaction->product_id }}"
                            ...
                            @endforeach
                            <tr>
                                <td colspan="8"><input type="hidden" name="qty"
                                        value="{{$transactions->sum('quantity')}}"></td>
                            </tr>
                            @endif

                            <!-- Total Discount Row -->
                            <tr class="bg-gray-50">
                                <td colspan="6" class="px-6 py-4 text-right text-sm font-medium text-gray-900">Total
                                    Discount:</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900"
                                    id="totalDiscountText">
                                    {{ isset($transactions) ? $transactions->sum('discount') : 0 }}
                                    <input type="hidden"
                                        value="{{ isset($transactions) ? $transactions->sum('discount') : 0 }}"
                            ...
                            <tr class="bg-gray-50">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-bold text-indigo-700"
                                    ...
                                    <input type="hidden"
                                        value="{{ isset($transactions) ? $transactions->sum('total_amount') : 0 }}"
                            ...
                            <tr>
                                <td colspan="8"
                                    class="px-6 py-3 bg-gradient-to-r from-blue-400 to-blue-600 text-center text-sm font-medium text-white">
                                    Customer Detail</td>
                            </tr>
                                ...
                                <td colspan="8" class="px-6 py-4">
                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                        <div>
                                            ...
                                            <input type="text"
                                                class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md py-2 px-3 border"
                                                id="customerMobile" name="mobile_number"
                                        ...
                                        <div>
                                            ...
                                            <input type="text"
                                                class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md py-2 px-3 border"
                                                id="customerName" name="name" placeholder="Enter customer name"
                                        ...
                                        @if(!isset($sale))
                                        <div>
                                            <label for="customerCNIC"
                                                class="block text-sm font-medium text-gray-700 mb-1">CNIC</label>
                                            <input type="text"
                                                class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md py-2 px-3 border"
                                                id="customerCNIC" name="cnic"
                                        ...
                                        <div>
                                            <label for="Cash"
                                                class="block text-sm font-medium text-gray-700 mb-1">Cash</label>
                                            <input type="number"
                                                class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md py-2 px-3 border"
                                                id="Cash" name="cash" value="{{ old('cash', $sale->cash ?? '') }}"
                                    ...
                                    <div class="mt-6">
                                        ...
                                        <textarea
                                            class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md py-2 px-3 border"
                                            id="customerAddress" rows="3" name="address"
...
$(document).ready(function() {
    ...
    $(document).on('click', '#saveTransactionButton', function(e) {
        ...
        if (service === '' || parseFloat(service) < 0) {
            Swal.fire("Missing or Invalid Service Charge",
                "Please enter a valid service charge (0 or more).", "warning");
            return;
        }
        ...
        if (saleMode === 'edit' && actionUrl === genericAddTransactionUrl) {
            ...
            if (currentSaleId) {
                ...
                if (saleIdMatch && saleIdMatch[1]) {
                    ...
                    console.error("Fallback: Could not determine sale_id from saleFormAction:",
                        saleFormAction);
                }
            }
        }
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if (!t || typeof t.id === 'undefined') {
                    console.error("Received invalid transaction data from server:",
                        response);
                    Swal.fire("Save Error",
                        "Received incomplete data from server. The page will reload.",
                        "error").then(() => {
                        location.reload();
                    });
                    return;
                ...
                if ($('#tableHolder').find('tr:has(th):contains("Total Discount")').length >
                    0) {
                    $('#tableHolder').find('tr:has(th):contains("Total Discount")').before(
                        newRowHtml);
                } else if ($('#tableHolder').find('tr:has(th):contains("Customer Detail")')
                    .length > 0) {
                    $('#tableHolder').find('tr:has(th):contains("Customer Detail")').before(
                        newRowHtml);
                } else {
    ...
    $(document).on('click', '.edit-sale', function() {
        ...
        let updateUrl = "{{ route('transaction.update', ['id' => ':id_placeholder']) }}".replace(
            ':id_placeholder', id);
    ...
    $(document).on('click', '#submitTransactionButton', function(e) {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if (row.length) {
                    row.find('td:eq(1)').text(t.products?.item_name || t.product_name ||
                        'N/A');
                    row.find('td:eq(2)').text(t.products?.selling_price || t.amount ||
                        'N/A');
    ...
    $(document).on('click', '.delete-sale', function() {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                let deleteUrl =
                    "{{ route('transaction.destroy', ['id' => ':id_placeholder']) }}".replace(
                        ':id_placeholder', transactionId);
                    ...
                    success: function(response) {
                        Swal.fire({
                            ...
                            text: response.message ||
                                "Sale line item deleted successfully!",
                            icon: "success",
                            timer: 1500,
                            showConfirmButton: false
                        });
                        ...
                        let totalQuantity = 0;
                        $('#tableHolder tr[data-id]').each(function() {
                            const qty = parseFloat($(this).find('td:eq(3)')
                                .text()) || 0;
                            const d = parseFloat($(this).find('td:eq(4)')
                                .text()) || 0;
                            const a = parseFloat($(this).find('td:eq(6)')
                                .text()) || 0;
                            totalQuantity += qty;
                            totalDiscount += d;
                            totalAmount += a;
                        ...
                        $('#tableHolder tr[data-id]').each(function(index) {
                            let currentTransactionId = $(this).find(
